...
public class Lexer implements Iterator<Token>, Iterable<Token> {
    private final Map<Token.Type, Pattern> patterns = Map.ofEntries(
            Map.entry(Token.Type.SubFactorial, Pattern.compile("!!")),
            Map.entry(Token.Type.Factorial, Pattern.compile("!")),
            Map.entry(Token.Type.ClosingParenthesis, Pattern.compile("\\)")),
            Map.entry(Token.Type.Division, Pattern.compile("/")),
            Map.entry(Token.Type.Minus, Pattern.compile("-")),
            Map.entry(Token.Type.Multiplication, Pattern.compile("\\*")),
            Map.entry(Token.Type.Natural, Pattern.compile("[0-9]+")),
            Map.entry(Token.Type.OpeningParenthesis, Pattern.compile("\\(")),
            Map.entry(Token.Type.Plus, Pattern.compile("\\+"))
    );
