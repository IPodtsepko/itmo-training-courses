...
Очереди делятся на две группы пополам - dynamics (1 - 15), real time (16 - 31). В очереди с приоритетом 0 обычно лежит 1 процесс, отвечающий за зануление страниц (хотя это может быть и не так).

Концепция: процессы реального времени имеют гарантированное время отклика, ОС не влияет на их расположение (если он попал в очередь 29, то как бы он себя не вел, он там и останется). Пользовательский процесс почти никогда не может подняться выше 15 очереди, потому что выше уже системные процессы, у которых приоритет выше. Он может оказаться там, если выполнить ручное изменение приоритета на 24, однако это крайне не рекомендуется.
