    ...
    bool enqueue(uint32_t seq_num, uint16_t count)
    {
        if (!m_running) {
            std::cout << m_name << " stopping, "
                      << ", ignoring expected_seq_num=" << seq_num
                      << ", count=" << count << "\n";

            LOG("stopping, ignoring expected_seq_num=" << seq_num << ", count=" << count);
            return false;
        }
        ...
        bool was_empty = false;
        {
            ...
            LOG("enqueued packet: expected_seq_num: " << seq_num << ", count=" << count);
        }
...
private:
    void run()
    {
        while (m_running) {
            size_t size = 0;
            {
                // try_wait_late_pockets for packets
                for (;;) {
                    {
                        std::unique_lock<std::mutex> lock(m_mutex);
                        size = m_queue.size();
